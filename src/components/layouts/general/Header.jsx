					...
					<div className="header-wrap">
						<div className="col-md-2">
							<div id="logo" className="logo">
                                <Link to="/" onClick={() => {window.location.href=this.state.linklogo}}>
									<img src="images/logo-blog.png" alt="codesnapr" width={157} height={29}
                                    data-retina="images/logo-blog.png" data-width={200} data-height={29} /></Link>
							</div>
                            {/* logo*/}
						</div>
                        {/* .col-md-2  */}
									...
									<ul className="menu">
										{
											menus.map(data =>(
												<li className={data.name === this.props.data.names ? "active" : ""} key={data.id}>
													<Link to={data.linkmenu} onClick={() => {window.location.href=data.linkmenu}}>{data.name} 
														{ data.namesub === undefined ? "" : <i className="fa fa-angle-down" aria-hidden="true"></i>}
													</Link>
													{
